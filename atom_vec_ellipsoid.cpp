...
void AtomVecEllipsoid::copy_bonus(int i, int j, int delflag)
{
  ...
  printf("delflag: %d, ellipsoid[j]: %d\n", delflag, ellipsoid[j]);
...
{
  int m = 0;
  printf("pack_exchange_bonus: nlocal, nlocal_bonus: %d, %d\n", atom->nlocal, nlocal_bonus);
  if (ellipsoid[i] < 0)
    buf[m++] = ubuf(0).d;
  else {
    buf[m++] = ubuf(1).d;
...
{
  int m = 0;
  printf("unpack_exchange_bonus: nlocal, nlocal_bonus: %d, %d\n", atom->nlocal, nlocal_bonus);
  if (ubuf(buf[m++]).i == 0)
    ellipsoid[ilocal] = -1;
  else {
...
{
  printf("set_shape: %d, %f, %f, %f\n", i, shapex, shapey, shapez);
  if (ellipsoid[i] < 0) {
